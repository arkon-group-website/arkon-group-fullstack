...
export function Header({ locale }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-arkon-line bg-white/92 shadow-sm backdrop-blur-xl">
      <div className="border-b border-arkon-line bg-arkon-pearl/92">
      ...
      <div className="arkon-container flex min-h-20 items-center justify-between gap-5">
        <Link href={localePath(locale)} className="flex items-center" aria-label="ARKON Group">
          <Image src="/logos/arkon-group-logo.svg" alt="ARKON Group" width={184} height={54} priority className="h-14 w-auto" />
        </Link>
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label={isArabic ? "التنقل الرئيسي" : "Primary navigation"}>
          {navigation.map((item) => {
            ...
            const hasMenu = item.href === "/companies" || item.href === "/services";
